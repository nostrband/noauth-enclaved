...
export class Nip44 {
  ...
  private getKey(privkey: Uint8Array, pubkey: string, extractable?: boolean) {
  ...
  public encrypt(privkey: Uint8Array, pubkey: string, text: string): string {
    const key = this.getKey(privkey, pubkey);
    return encryptNip44(text, key);
  }

  public decrypt(privkey: Uint8Array, pubkey: string, data: string): string {
    const key = this.getKey(privkey, pubkey);
